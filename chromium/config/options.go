...
//
// Options 需被多个实现包读取（启动器、Browser、档案归属校验），放在此处使它们都能向下依赖。
// Options 类型本身不对外暴露（门面只转发 Option 与 WithXxx），调用方无法自行构造配置。
package config
...
//
// 字段导出是为了让内部各实现包能读取；类型本身不导出，因此调用方只能通过 WithXxx 声明式配置。
type Options struct {
	ChromePath    string
	...
	// TrustExistingBrowser 跳过「端口上的浏览器是不是目标」核实，仅供手动启动的 Chrome 显式声明。
